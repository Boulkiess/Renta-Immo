...
function exportCSV(d) {
  ...
  KEYS.forEach(k => {
    ...
    row('Année', isLoc ? 'Loyers (€)' : 'Loyer éco. (€)', 'Charges (€)', 'Annuité+Ass (€)',
      ...(isLoc ? ['Impôts (€)'] : []), 'CF net (€)', 'CF cumulé (€)', 'Valeur bien (€)',
      'Capital restant (€)', 'Patrimoine total (€)', 'ETF poche (€)', 'Revente nette (€)', 'Bilan (€)', 'ETF pur (€)');
    s.computed.flux.forEach((f, i) => {
      row(f.annee, isLoc ? f.loyersEncaisses : f.loyerEconomise,
        ...
        f.patrimoineTotal ?? '', f.etfPoche ?? '', f.reventeNette, f.bilanRevente,
...
export function doExport(fmt, data) {
  ...
  if (fmt === 'json')      dlFile(`immorenta_${ts}.json`, JSON.stringify(data, null, 2), 'application/json');
...
function applyGlobalFromObj(g, updateG) {
  const map = {
    inflation: v => +v, regime: v => String(v), horizon: v => +v, tauxActu: v => +v,
    rendAlt: v => +v, loyerPerso: v => +v, revalLoyerPerso: v => +v, budgetMensuel: v => +v,
    investirSurplus: v => v === true || v === 'true',
    apportETF: v => +v,
  };
  const updates = {};
  Object.entries(map).forEach(([k, fn]) => { if (g[k] != null) updates[k] = fn(g[k]); });
  ...
  reader.onload = e => {
    try {
      ...
      if (ext === 'json') {
        ...
        const g = {};
        ['inflation','regime','horizon','tauxActu','rendAlt','loyerPerso','revalLoyerPerso','budgetMensuel','investirSurplus','apportETF']
          .forEach(k => { if (flat[`global.${k}`] != null) g[k] = flat[`global.${k}`]; });
        applyGlobalFromObj(g, updateG);
        KEYS.forEach(k => {
          const data = { label: flat[`simulations.${k}.label`], enabled: flat[`simulations.${k}.enabled`], inputs: {} };
          Object.keys(mkDef('loc')).forEach(f => { const v = flat[`simulations.${k}.inputs.${f}`]; if (v != null) data.inputs[f] = v; });
