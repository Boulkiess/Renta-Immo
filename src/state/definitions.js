...
if (typeof window !== 'undefined') {
  ...
  if ((now.getFullYear() - cy) * 12 + now.getMonth() + 1 - cm > 12)
    console.warn(
      `[ImmoRenta] Rate curve last updated ${TAUX_CURVE_UPDATED_AT} — consider refreshing _TAUX_CURVE in definitions.js`
    );
}
...
export function computeAutoValue(p, fieldKey) {
  ...
  switch (fieldKey) {
    ...
    case 'taux':
      return (p.duree ?? 0) > 0 ? +interpTaux(p.duree).toFixed(2) : 0;
    case 'provision':
