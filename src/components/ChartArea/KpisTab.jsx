...
const TableWrap = styled.div`flex: 1; padding: 12px 12px 0;`;

const TableTitle = styled.div`font-size: 13px; font-weight: 700; margin-bottom: 3px;`;
const TableDesc = styled.div`font-size: 11px; color: ${({ theme }) => theme.muted}; margin-bottom: 10px; line-height: 1.5;`;
...
const DataCell = styled.td`
  ...
  font-family: ${({ theme }) => theme.mono}; font-size: 12px;
...
const Card = styled.div`
  flex: 1; border: 1px solid ${({ theme }) => theme.border}; border-radius: 8px;
  border-top: 3px solid ${({ $col }) => $col}; padding: 12px 14px;
`;
const CardLabel = styled.div`font-size: 11px; color: ${({ theme }) => theme.muted}; margin-bottom: 4px;`;
const CardValue = styled.div`
  font-family: ${({ theme }) => theme.mono}; font-size: 20px; font-weight: 700;
  color: ${({ $col }) => $col};
`;
const CardSub = styled.div`font-size: 11px; color: ${({ theme }) => theme.muted}; margin-top: 2px;`;

/* helpers */
const parseNum = (str) => {
  if (str == null || str === '—') return null;
  return parseFloat(String(str).replace(/ /g, '').replace(/\s/g, '').replace(',', '.').replace(/[^\d.\-]/g, ''));
};
  ...
  const sections = [
    {
      ...
      rows: [
        ...
        { label: t('kpisTable.mensualiteAss'), fmt: k => fmtE(RES[k].mens + RES[k].assM), better: 'min', neg: false },
        { label: t('kpisTable.coutCredit'),    fmt: k => fmtE(RES[k].totInt + RES[k].totAss), better: 'min', neg: false },
      ],
    },
    {
      cat: t('kpisTable.rendBrut').startsWith('R') ? 'Rendements & Cashflow' : 'Yields & Cash-flow',
      rows: [
        { label: t('kpisTable.rendBrut'),  fmt: k => sims[k].mode === 'loc' ? fmtP(RES[k].rendBrut) : '—', better: 'max', neg: false },
        { label: t('kpisTable.rendNet'),   fmt: k => sims[k].mode === 'loc' ? fmtP(RES[k].rendNet) : '—',  better: 'max', neg: false },
        { label: t('kpisTable.cfMensuel'), fmt: k => sims[k].mode === 'loc' ? fmtE(RES[k].cfM) : '—',     better: 'max', neg: true },
        { label: t('kpisTable.effortRP'),  fmt: k => sims[k].mode === 'rp' ? fmtE(Math.abs(RES[k].cfM)) : '—', better: 'min', neg: false },
    ...
    },
    {
      ...
      rows: [
        ...
        { label: t('kpisTable.van', { tauxActu: G.tauxActu, horizon: hz }), fmt: k => fmtE(RES[k].van), better: 'max', neg: true },
        { label: t('kpisTable.moic', { horizon: hz }),  fmt: k => fmtMoic(RES[k].moic), better: 'max', neg: false },
      ],
    },
    {
      cat: t('kpisTable.patTotal', { horizon: hz }).startsWith('P') ? 'Patrimoine' : 'Wealth',
      rows: [
        { label: t('kpisTable.patNet', { horizon: hz }),    fmt: k => fmtE(RES[k].flux[hz - 1]?.patNet),    better: 'max', neg: true },
        { label: t('kpisTable.patTotal', { horizon: hz }),  fmt: k => fmtE(RES[k].flux[hz - 1]?.patTotal),  better: 'max', neg: true },
        { label: t('kpisTable.patTotal30'),                 fmt: k => fmtE(RES[k].flux[29]?.patTotal),      better: 'max', neg: true },
        { label: t('kpisTable.etfPurHorizon', { horizon: hz }), fmt: () => fmtE(etfHz),                     better: null,  neg: false },
        { label: t('kpisTable.etfPur30'),                        fmt: () => fmtE(etf30),                    better: null,  neg: false },
        { label: t('kpisTable.avantageEtf', { horizon: hz }),    fmt: k => fmtE((RES[k].flux[hz - 1]?.patTotal ?? 0) - (etfHz ?? 0)), better: 'max', neg: true },
        { label: t('kpisTable.crossover'),  fmt: k => fmtCross(crossovers[k]),                               better: 'min', neg: false },
      ],
    },
  ];
    ...
    <Wrap>
      <TableWrap>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ textAlign: 'left', padding: '6px 10px', borderBottom: '1px solid var(--border)', fontSize: 10, fontWeight: 600, color: 'var(--muted)' }}>
                {t('kpisTable.indicator')}
              </th>
              {activeKeys.map(k => (
                <th key={k} style={{ textAlign: 'right', padding: '6px 10px', borderBottom: '1px solid var(--border)', fontSize: 11, fontWeight: 800, color: COL[k] }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: COL[k], display: 'inline-block' }} />
                    {sims[k].label}
                  </span>
                </th>
              ))}
            ...
            {sections.map(sec => [
              ...
              ...sec.rows.map(row => {
                ...
                if (row.better && activeKeys.length > 1) {
                  ...
                  if (valid.length > 1) {
                    const target = row.better === 'max' ? Math.max(...valid.map(v => v.n)) : Math.min(...valid.map(v => v.n));
