...
// ── ETF pur reference (global, cleared in-place on recompute) ──
export const etfPurGlobal = [];

export function recomputeETFPur(){
  etfPurGlobal.length = 0;
  let cap = G.apportETF;
...
export function compute(p){
  ...
  for(let yr=1; yr<=30; yr++){
    ...
    flux.push({yr,le,chg,ann:ann+asp,imp,cfN,cfC,vb,rest,
      ...
      reventeNet,bilanRevente,bilanTotal,pr});
...
// ── Crossover: année où patTotal dépasse ETF pur ──────────
export function crossoverYear(res){
